...
const features = [
  { icon: '◈', title: 'Приватный сервер', desc: 'Вход только по проходкам. Никаких случайных людей — только своя атмосфера и доверенное сообщество.' },
  { icon: '⚔', title: 'Semi-RP', desc: 'Живой мир с историей и событиями. Погружайся настолько глубоко, насколько хочешь.' },
  { icon: '⬡', title: 'Свои дополнения', desc: 'Debug палочка, невидимый свет, невидимые рамки, динамическая карта и многое другое.' },
  { icon: '◎', title: 'Без доната', desc: 'Никакого pay-to-win. Все равны. Прогресс зависит только от твоего времени и умений.' },
]
    ...
    <main style={{ background: '#0a0a0c', minHeight: '100vh', color: '#fff' }}>
      ...
      <section id="about" style={{ padding: '0 60px 80px', maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 40 }}>
          <div style={{ width: 24, height: 1, background: 'rgba(255,255,255,0.2)' }} />
          <span style={{ fontSize: 11, letterSpacing: '0.28em', color: 'rgba(255,255,255,0.28)', textTransform: 'uppercase', fontFamily: 'monospace' }}>Особенности</span>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, overflow: 'hidden' }}>
          {features.map(({ icon, title, desc }, i) => (
            <div key={title} style={{ padding: '52px 52px', background: 'rgba(255,255,255,0.02)', borderRight: i % 2 === 0 ? '1px solid rgba(255,255,255,0.06)' : 'none', borderBottom: i < 2 ? '1px solid rgba(255,255,255,0.06)' : 'none', transition: 'background .2s' }}
              onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,255,255,0.04)')}
              onMouseLeave={e => (e.currentTarget.style.background = 'rgba(255,255,255,0.02)')}>
              <div style={{ fontSize: 20, marginBottom: 18, opacity: .5 }}>{icon}</div>
              <div style={{ fontSize: 17, fontWeight: 500, marginBottom: 12 }}>{title}</div>
              <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.36)', lineHeight: 1.8, fontWeight: 300 }}>{desc}</div>
            </div>
          ))}
        </div>
      </section>
